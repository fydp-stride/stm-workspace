...
{
    if (HAL_I2C_Mem_Read(hi2c, (uint16_t)(BQ72441_I2C_ADDRESS << 1), reg, 1, data, len, 50) == HAL_OK)
        return true;
    else
        return false;
}
...
{
    if (HAL_I2C_Mem_Write(hi2c, (uint16_t)(BQ72441_I2C_ADDRESS << 1), reg, 1, data, len, 50) == HAL_OK)
        return true;
    else
        return false;
}
 ...
 * */
bool BQ27441_init(I2C_HandleTypeDef *dev_hi2c) {
    if (dev_hi2c == NULL)
        return false;
