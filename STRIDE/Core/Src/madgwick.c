...
{
    // due to slow initial convergence rate of filter, we initialize quaternions
    // using an estimate of the initial orientation derived from acceleration and
    // magnetic field
    float roll = -atan2f((-ay) , sqrt(ax * ax + az * az));
    ...
    handle.beta = beta;
    handle.q0 = cr * cp * cy + sr * sp * sy;
    handle.q1 = sr * cp * cy - cr * sp * sy;
    handle.q2 = cr * sp * cy + sr * cp * sy;
    handle.q3 = cr * cp * sy - sr * sp * cy;
}
...
void madgwick_get_angles(madgwick_angle_data_t *angle_data) {
    ...
    float q2 = handle.q2;
    float q3 = handle.q3;
    angle_data->x = atan2f(q0*q1 + q2*q3, 0.5f - q1*q1 - q2*q2);
	angle_data->y = asinf(-2.0f * (q1*q3 - q0*q2));
	angle_data->z = atan2f(q1*q2 + q0*q3, 0.5f - q2*q2 - q3*q3);

    // radians to degrees
    angle_data->x *= 57.2957795147f;
    angle_data->y *= 57.2957795147f;
    angle_data->z *= 57.2957795147f;
}
